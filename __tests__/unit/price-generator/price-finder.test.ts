...
describe("price-finder", () => {
    ...
    describe("findPrice", () => {
        test("should find VAT price for the given date", () => {
            expect(findPrice(new Date("2023-07-20T10:00:00Z"), results)).toEqual(2);
            expect(findPrice(new Date("2023-07-21T10:00:00Z"), results)).toEqual(4);
            expect(findPrice(new Date("2023-07-21T23:00:01Z"), results)).toEqual(6);
        });

        test("should return undefined if date is not found", () => {
            expect(findPrice(new Date("2023-07-19T22:59:59Z"), results)).toBeUndefined();
            expect(findPrice(new Date("2023-07-22T23:00:01Z"), results)).toBeUndefined();
            expect(findPrice(new Date("2023-07-23T10:00:00Z"), results)).toBeUndefined();
        });
    });
});
